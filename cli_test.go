...
func TestRun__versionFlag(t *testing.T) {
	...
	args := strings.Split("mysql-yaml-loader -version", " ")

	exitCode := cli.Run(args)
	if exitCode != ExitCodeOK {
		t.Errorf("Expected %v, but %v", ExitCodeOK, exitCode)
	}

	expected := fmt.Sprintf("%s version %s", AppName, Version)
	if !strings.Contains(errStream.String(), expected) {
		t.Errorf("Expected %v, but %v", expected, errStream.String())
	}
}
...
func TestRun__helpFlag(t *testing.T) {
	...
	if !strings.Contains(outStream.String(), usage) {
		t.Errorf("Expected %v, but %v", usage, outStream.String())
	}
}
...
func TestRun__parseError(t *testing.T) {
	...
	if exitCode != ExitCodeFlagParseError {
		t.Errorf("Expected %v, but %v", ExitCodeFlagParseError, exitCode)
	}
